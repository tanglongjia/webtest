...
(function($){
    ...
    BootstrapTable.prototype.init = function () {
       ...
       // alert(that.options.editExceptRow);
       // console.log(that.options.editExceptRow);
    ...
    BootstrapTable.prototype.initEdit = function(){
    	...
    	this.$body.find('> tr').unbind('click').on('click',function(){
    		...
    		//console.log($tr);
    		//alert(that.options.editExceptRow);
    		//alert($tr.context.sectionRowIndex);
    		if($.inArray($tr.context.sectionRowIndex, that.options.editExceptRow)>-1){
    			return;
    		}
...
//			if(that.prevEditRow.hasClass('editable-insert')){
...
//    		}
			that.prevEditRow.find('td').closest('td').siblings().html(function(i,html){
				...
				if(textVal != undefined){
					if($(this).data('oldVal') != (hiddenVal?hiddenVal:$.trim(textVal)) && $(this).data('field')) {
						...
						isModi = true;
						console.log(that.prevEditRow.data('index'));
						console.log($(this).data('field'));
						
						if(typeof that.options.onDataChangeFunc == 'function'){
							//that.options.onDataChangeFunc($(this).context.parentElement.sectionRowIndex,$(this).context.cellIndex);
							//that.options.onDataChangeFunc($(this).context.parentElement.sectionRowIndex,$(this).context.cellIndex);
							//editRow = $(this).context.parentElement.sectionRowIndex;
							//editCol = $(this).context.cellIndex;
							editRow.push($(this).context.parentElement.sectionRowIndex);
							editCol.push($(this).context.cellIndex);
						}
    		...
    		if(isModi || that.prevEditRow.hasClass('editable-insert')){
    			...
    			if(typeof that.options.onDataChangeFunc == 'function'){
					//that.options.onDataChangeFunc($(this).context.parentElement.sectionRowIndex,$(this).context.cellIndex);
					that.options.onDataChangeFunc(editRow,editCol);
				}
    		}
    		else{
